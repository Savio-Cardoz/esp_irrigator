...
esp_err_t nvs_read_chunk(const char *key, void *data, size_t len)
{
    ...
    if ((err == ESP_ERR_NVS_NOT_FOUND) || (err != ESP_OK))
    {
        ESP_LOGE("non_volatile_storage", "Failed to get blob length from NVS: %s", esp_err_to_name(err));
        nvs_close(handler);
        return err;
    }
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE("non_volatile_storage", "Failed to read chunk from NVS: %s", esp_err_to_name(err));
    }
    else
    {
        ESP_LOGI("non_volatile_storage", "Chunk read successfully from NVS with key: %s", key);
    }
    nvs_close(handler);
    return err;
}
...
void nvs_write_chunk(const char *key, const void *data, size_t len)
{
    nvs_handle_t my_handle;
    esp_err_t err;
    err = nvs_open(NON_VOLATILE_STORAGE, NVS_READWRITE, &my_handle);
    if (err != ESP_OK)
    {
        ESP_LOGE("non_volatile_storage", "Failed to open NVS: %s", esp_err_to_name(err));
        return;
    }
    err = nvs_set_blob(my_handle, key, data, len);
    if (err != ESP_OK)
    {
        ESP_LOGE("non_volatile_storage", "Failed to write chunk to NVS: %s", esp_err_to_name(err));
        nvs_close(my_handle);
        return;
    }
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE("non_volatile_storage", "Failed to commit NVS changes: %s", esp_err_to_name(err));
    }
    else
    {
        ESP_LOGI("non_volatile_storage", "Chunk written successfully to NVS with key: %s", key);
    }
    nvs_close(my_handle);
}